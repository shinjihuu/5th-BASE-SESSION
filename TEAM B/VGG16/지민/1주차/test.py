...
# 테스트 데이터셋 로딩
test_data = datasets.ImageFolder('./Data/cat_dog/test_set', transform=transform)
...
with torch.no_grad(): # 그래디언트 계산을 비활성화하여 메모리 사용량 줄이고 계산 속도 향상
    for i, [image, label] in enumerate(test_loader):
        ...
        total += label.size(0) # 처리된 테스트 데이터 수 갱신
        correct += (output_index == y).sum().float() # 정확히 예측된 데이터 수 갱신
